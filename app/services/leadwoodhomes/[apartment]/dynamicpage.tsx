        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-20 pt-32 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 grid gap-8 lg:grid-cols-[minmax(0,1.1fr)_360px]">
            <div className="max-w-3xl">
              ...
              {apartment.airbnb ? (
                <div className="mt-4 rounded-2xl border border-[#f5d6b3] bg-[#fff6e8] p-4 text-[#4f2f14]">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#b56320]">
                    Airbnb middleman request
                  </p>
                  <p className="mt-2 text-sm">
                    This is an Airbnb reference property. G&V can act as the middleman and book
                    for you on your behalf.
                  </p>
                  <div className="mt-3 flex items-center gap-2">
                    <span className="rounded-full bg-[#f7e3cf] px-2 py-1 text-[11px] font-semibold text-[#7e4308]">
                      {apartment.airbnb.type === "home" ? "Home" : "Apartment"}
                    </span>
                    <a
                      href={apartment.airbnb.url}
                      target="_blank"
                      rel="noreferrer"
                      className="text-sm font-semibold text-[#dd5500] underline"
                    >
                      View Airbnb listing
                    </a>
                  </div>
                  <div className="mt-3 space-y-2 text-xs text-[#5a3c22]">
                    <p>Owner / host contact: {apartment.airbnb.ownerContact}</p>
                    <p>Location: {apartment.airbnb.location}</p>
                  </div>
                  <Link
                    href={`/contact-us?source=leadwood&home=${encodeURIComponent(apartment.title)}`}
                    className="mt-3 inline-flex items-center justify-center rounded-full bg-[#1b1209] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#dd5500]"
                  >
                    Contact us to do booking as middleman
                  </Link>
                </div>
              ) : null}
            </div>
